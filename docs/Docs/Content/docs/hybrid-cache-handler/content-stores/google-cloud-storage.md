---
title: Google Cloud Storage HTTP cache content store
sidebarLabel: Google Cloud Storage
description: GCS resumable uploads, bounded downloads, cancellation and integrity behavior, retention, and opt-in tests.
order: 4
...
[NuGet](https://www.nuget.org/packages/DamianH.HttpHybridCacheHandler.ContentStore.GoogleCloudStorage/) |
[Source](https://github.com/damianh/http-libs/tree/main/hybrid-cache-handler/src/HttpHybridCacheHandler.ContentStore.GoogleCloudStorage)
...
credentials, retry settings, access controls, or lifecycle rules. See
...
`storage.buckets.get` permissions. Run from the repository root:
